...
TRANSACTION_ACTIONS = (ACTION_INITIAL_CLAIM, ACTION_CLAIM, ACTION_BET_FLIP, ACTION_BET_ROLL, ACTION_GIFTER,
                       ACTION_GIFTEE)
...
class Currency(commands.Cog):
    ...
    async def initial_claim(self, ctx):
        ...
        c.execute("SELECT IFNULL(MAX(Date), 0) FROM BankTransactions "
                  "WHERE UserID = ? AND Action = ?", (ctx.message.author.id, ACTION_INITIAL_CLAIM))
        ...
        await self.create_bank_transaction(c, ctx.message.author, self.currency["initial_amount"], ACTION_INITIAL_CLAIM,
                                           metadata)
    ...
    async def claim(self, ctx):
        ...
        c.execute("SELECT IFNULL(MAX(Date), 0) FROM BankTransactions "
                  "WHERE UserID = ? AND Action = ?", (ctx.message.author.id, ACTION_CLAIM))
        ...
        if last_claimed < threshold:
            ...
            await ctx.send("Please wait {}h {}m to claim again!".format(time_left.seconds // 3600,
