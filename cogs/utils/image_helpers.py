...
def apply_transform(transform, buffer, size, max_size, ext, is_png, *args):
    ...
    if ratio:
        _, buffer = cv2.imencode(
            f".{ext}", result, (cv2.CV_IMWRITE_PNG_COMPRESSION if is_png else cv2.IMWRITE_JPEG_QUALITY, ratio)
        )
...
async def filter_image(loop, transform, ctx, history_limit, max_size, *args):
    ...
    if att is None:
        await ctx.send(
            "no image could be found (only attached image files" " can be detected) or message could not be found"
        )
        return
    ...
    try:
        with concurrent.futures.ProcessPoolExecutor() as pool:
            buffer = await loop.run_in_executor(
                pool, partial(apply_transform, transform, buffer, att.size, max_size, ext, is_png, *args)
            )
...
def polar(image):
    return np.rot90(cv_linear_polar(image, cv2.INTER_LINEAR + cv2.WARP_FILL_OUTLIERS), -1)


def cart(image):
    return cv_linear_polar(np.rot90(image), cv2.INTER_LINEAR + cv2.WARP_FILL_OUTLIERS + cv2.WARP_INVERSE_MAP)
...
def rblur(image, radius: int, max_rad: int):
    ...
    image = cv2.blur(image, (radius, 1))
    image = cv_linear_polar(image, cv2.INTER_LINEAR + cv2.WARP_FILL_OUTLIERS + cv2.WARP_INVERSE_MAP)
    return image
...
def deepfry(image, iterations: int, max_iter: int):
    iterations = max(0, min(iterations, max_iter))
    kernel = np.array([[0, 0, 0], [0, 1, 0], [0, 0, 0]]) + np.array([[0, -1, 0], [-1, 4, -1], [0, -1, 0]]) * 0.3
