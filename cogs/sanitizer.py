...
TIKTOK_MOBILE = re.compile(r"(https?:\/\/m\.tiktok\.com\/v\/[0-9]*)\.html\?[A-Za-z0-9_&=%\.\?-]*")
TIKTOK_DESKTOP = re.compile(r"(https?:\/\/www\.tiktok\.com\/@[A-Za-z0-9_\.]*\/video\/[0-9]*)\?[A-Za-z0-9_&=%\.\?-]*")

def unroll_tiktok(link):
    return requests.head(link, headers={"User-Agent": "Mozilla/5.0 (X11)"}, allow_redirects=True).url
...
class Sanitizer(commands.Cog):
    ...
    async def tiktok_link_sanitizer(self, msg):
        if (not isinstance(msg.author, discord.Member)) or utils.get(msg.author.roles, name=self.bot.config.sanitation["tt_optout"]):
            return
        replace: bool = False
        ...
        if replace:
            ...
            if not utils.get(msg.author.roles, name=self.bot.config.sanitation["tt_silent"]):
                dm_channel = msg.author.dm_channel or await msg.author.create_dm()
                await dm_channel.send(f"WARNING: a message you sent contained a tiktok link"
