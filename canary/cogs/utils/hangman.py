...
def mk_animal_list() -> list[tuple[str, str]]:
    ...
    for i in range(16, len(animal_list_soup)):
        ...
        while str(img_list[img_index]["src"]).endswith(".svg.png"):
            img_index += 1
        animal_list.append((animal_name["title"].split(" (")[0], "https:" + img_list[img_index]["src"]))
    return animal_list
...
class HangmanState:
    def __init__(self, category_name: str, word_list: list[tuple[str, Optional[str]]]):
        self.word, self.img = random.choice(word_list)
        self.lword = self.word.lower()
        self.not_guessed: set[str] = set(char for char in self.lword if char in "abcdefghijklmnopqrstuvwxyz")
        self.previous_guesses: set[str] = set()
        self.field_name: str = f"hangman (category: {category_name})"
        self.first_line: str = " ".join(
            char if lowered_char not in self.not_guessed else "_" for char, lowered_char in zip(self.word, self.lword)
        )
        self.last_line: str = "previous guesses: "
        self.player_msg_list: list[str] = []
        self.num_mistakes: int = 0
        self.embed = (
            discord.Embed(colour=random.randint(0, 0xFFFFFF))
            .add_field(name=self.field_name, value=f"`{self.first_line}`\n```{HANG_LIST[self.num_mistakes]}```")
    ...
    def add_msg(self, new_msg):
        ...
        self.embed.set_field_at(
            ...
            value="`{0}`\n```{1}```\n{2}".format(
                self.first_line, HANG_LIST[self.num_mistakes], "\n".join(self.player_msg_list)
            ),
        )
