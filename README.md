...
2. Dependencies are managed with `poetry` (1.8.3+), which can be installed via `pip` with:
...
6. You may enter the virtual environment generated by the Poetry installation with: `poetry shell` or simply run the 
...
configuration object described in [config.py](canary/config/config.py). These environment variables
correspond to the properties of the object, uppercased, underscore-spaced, and prefixed with `CANARY_.`

A bare minimum set of environment variables to set can be found in [example.env](./example.env).
These variables must be set inside the container in order for the bot to function.
